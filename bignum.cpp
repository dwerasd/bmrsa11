...
{
	const size_t nLen = strlen(szSourceVal);
	size_t nIdx;
	const char* pChar;
	CBigNum Pow10;
...
CBigNum CBigNum::FromByteString(const char* szIn, const unsigned int nLength)
{
	...
	if (!nLength)
		nUseLength = static_cast<DWORD>(strlen(szIn));
	else
		nUseLength = nLength;
...
CBigNum CBigNum::FromBase64String(const char* szIn)
{
	...
	const int nCount = static_cast<int>(sizeof(szBase64) - 1);
	for (i = 0; i < nCount; ++i)
	{
		szCharMap[szBase64[i]] = static_cast<unsigned char>(i);
	}
	szCharMap['='] = 0;
...
CBigNum::operator CBigNumString() const
{
	...
	{
		TempBigNum[nDigit] = static_cast<char>('0' + (outval % 10U));
	}
